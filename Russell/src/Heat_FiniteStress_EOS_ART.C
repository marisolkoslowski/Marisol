#include "Heat_FiniteStress_EOS_ART.h"

registerMooseObject("SolidMechanicsApp",Heat_FiniteStress_EOS_ART);

InputParameters
Heat_FiniteStress_EOS_ART::validParams()
{
...
Heat_FiniteStress_EOS_ART::Heat_FiniteStress_EOS_ART(const InputParameters & parameters)
...
Real
Heat_FiniteStress_EOS_ART::computeQpResidual()
{
  ...
  //Thermomechanical Coupling
    Real q_eos;
    q_eos = -_u[_qp] * _gamma * _density[_qp] * _specific_heat[_qp] * mechanical_strain_trace_dot;
    q_tot += q_eos;    
...
Real
Heat_FiniteStress_EOS_ART::computeQpJacobian()
{
  Real mechanical_strain_trace;
  Real total_strain_trace;
  ...
  Real q_eos;
  q_eos = - _u[_qp] * _gamma * _density[_qp] * _specific_heat[_qp] * mechanical_strain_trace_dot;
  q_tot += q_eos; //Check abs

  // compute artificial viscosity stress and heat source terms
  Real J;
  J = 1 + _mechanical_strain[_qp].trace();  
  if (J < 1.0) {
      q_tot += std::abs(_beta_av * _stress_artificial[_qp].doubleContraction(total_strain_trace_dot));  // to ensure that only when in compression the av term contributes to heat generation
  }
